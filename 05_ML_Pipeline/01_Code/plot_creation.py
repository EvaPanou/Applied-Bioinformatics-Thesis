...
def ensure_figures_dir(out_dir: Path) -> Path:
    """
    Ensure the output folder exists and return it - figures save directly
    into it, no separate subfolder (matches FIGURES_DIR in configurations.py).
    """
...
def plot_benchmark_boxplot(
    ...
    if sns is not None:
        ...
        ax.boxplot(grouped, labels=combo_order, patch_artist=True)
    ...
    fig.tight_layout()
    save_figure(fig, fig_dir / "02_benchmark_boxplot.png", log)
...
def plot_benchmark_barplot(
    ...
    save_figure(fig, fig_dir / "02_benchmark_barplot.png", log)

# Benchmark Heatmap: in order to compare all the FS-classifier combination's mean AUC to find which is the best one
def plot_benchmark_heatmap(
    ...
    save_figure(fig, fig_dir / "02_benchmark_heatmap.png", log)


# Leaderboard: ranked view of all FS-classifier combos, winner highlighted
def plot_benchmark_leaderboard(
    ...
    *,
    ...
    """
    Horizontal ranked leaderboard of FS/classifier combinations by mean AUC.

    Unlike plot_benchmark_barplot, this shows explicit rank numbers,
    highlights the #1 combination distinctly, and annotates each bar with
    its fold count - a combination that got skipped in some outer folds
    (e.g. a feature selector that chose too few genes that fold) has a
    mean based on fewer folds than the others, which is worth seeing
    directly rather than only in the underlying CSV.
    ...
    - mean
    - std (optional but expected)
    - count (optional - number of folds the mean is based on)
    """
    log = get_log(log)

    if summary is None or summary.empty:
        log.warning("plot_benchmark_leaderboard skipped: summary is empty")
        return

    needed = {"fs_method", "classifier", "mean"}
    missing = needed - set(summary.columns)
    if missing:
        log.warning("plot_benchmark_leaderboard skipped: missing columns %s", sorted(missing))
        return

    df = summary.sort_values("mean", ascending=False).head(top_n).reset_index(drop=True)
    df["combo"] = df["fs_method"].astype(str) + " + " + df["classifier"].astype(str)
    df["rank"] = df.index + 1
    ...
    colors = [CB_ORANGE if i == 0 else CB_BLUE for i in range(len(df))]
    y_pos = np.arange(len(df))[::-1]  # rank 1 at the top

    xerr = df["std"].fillna(0).values if "std" in df.columns else None
    ax.barh(y_pos, df["mean"], xerr=xerr, color=colors, alpha=0.9, capsize=3)

    labels = [f"{r}. {c}" for r, c in zip(df["rank"], df["combo"])]
    ax.set_yticks(y_pos)
    ax.set_yticklabels(labels)

    for y, mean, std, n in zip(
        y_pos,
        df["mean"],
        df["std"] if "std" in df.columns else [None] * len(df),
        df["count"] if "count" in df.columns else [None] * len(df),
    ):
        text = f"{mean:.3f}" + (f" \u00b1 {std:.3f}" if std is not None else "")
        if n is not None:
            text += f" (n={int(n)})"
        ax.text(mean + 0.005, y, text, va="center", fontsize=9)

    ax.set_xlim(0.0, 1.0)
    ax.set_xlabel("Mean AUC")
    ax.set_title(f"Benchmark leaderboard (top {len(df)})")
    ax.grid(axis="x", alpha=0.3)

    save_figure(fig, fig_dir / "02_benchmark_leaderboard.png", log)
...
def plot_stability_barplot(
    ...
    - bootstrap_selection_freq, bootstrap_freq, stability, frequency, fold_frequency
    """
    log = get_log(log)
    ...
    value_col = None
    for c in ["bootstrap_selection_freq", "bootstrap_freq", "stability", "frequency", "fold_frequency"]:
        if c in df.columns:
            value_col = c
            break
...
def plot_final_panel_correlation_heatmap(
    ...
    fig_dir = ensure_figures_dir(out_dir)
    fig, ax = plt.subplots(figsize=(max(6, 0.6 * len(panel_genes)), max(5, 0.6 * len(panel_genes))))
    ...
    ax.set_title("Final panel Pearson correlation heatmap")

    save_figure(fig, fig_dir / "03_final_panel_correlation_heatmap.png", log)
