...
microarray data, starting from a 44-gene candidate set (Method_All_Union)
produced by the upstream 3-method DGE consensus (Methods A/B/C).

Main stages:
1. Setup and logging
2. Data loading and sealed donor split
3. Feature selection and outer cross-validation benchmarking
4. Final signature construction and bootstrap Boruta stability
...
def main() -> None:  # This function returns no actual values; it just formulates and executes the workflow
    ...
    Bootstrap-related methodology checkpoints (both are implemented):
    1. Final-panel bootstrap Boruta stability (resample donors, rerun
       Boruta, keep genes it confirms consistently).
    2. Held-out bootstrap AUC evaluation.
    """

    # Create the output directory (and any missing parent folders) if it does not already exist.
    ...
    # Bootstrap step 3: held-out AUC CI + development-set permutation test, runs at the end
    log.info(
    ...
    plots.plot_benchmark_leaderboard(summary, out_dir, log)  # Create a ranked leaderboard of FS/classifier combinations
    ...
    # Full Pearson correlation matrix for the ranked stable genes - reuses
    # the matrix finalise_panel already computed while doing its Pearson
    # pruning, instead of recomputing it from scratch here.
    corr_full = final["correlation_matrix"]
    corr_full.to_csv(
    ...
    stability_series = final.get("stability", pd.Series(dtype=float))
    ...
    # Build a per-gene table of every stable candidate, ordered by FS rank,
    # with its bootstrap frequency and whether it made the final panel.
    ...
    # Mark which of these genes made it into the final panel
    ...
    ci_info = helpers.bootstrap_auc_ci(
        ...
        y_proba=np.asarray(holdout_metrics["y_proba"]),
        n_bootstrap=config.N_HOLDOUT_BOOTSTRAP,
        seed=config.SEED,
        logger=log,
    )
    ...
    permutation_results = helpers.permutation(
        ...
        y_dev=donors_split["y_dev"],
        donors_dev=donors_split["donors_dev"],
        final_signature=fitted_model["signature"],
        classifier_name=fitted_model["classifier"],
        best_params=fitted_model["best_params"],
        logger=log,
        n_permutations=config.N_PERMUTATIONS,
        n_folds=config.N_INNER_FOLDS,
        seed=config.SEED,
    )
    ...
    if final["winner"]["classifier"] != "logreg":
        ...
        try:
            ...
            lr_ci = helpers.bootstrap_auc_ci(
                y_true=np.asarray(lr_holdout["y_true"]),
                y_proba=np.asarray(lr_holdout["y_proba"]),
                n_bootstrap=config.N_HOLDOUT_BOOTSTRAP,
                seed=config.SEED,
                logger=log,
            )
    ...
    if not shap_df.empty and "HAS_SHAP" in globals() and HAS_SHAP:
        ...
        # final_shap_values and Xs_df were already computed above when
        # building shap_df - reused directly here instead of re-transforming
        # the held-out data and re-reading them back from the saved CSV.
        try:
